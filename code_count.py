...
labelsPath = os.path.sep.join(["params_", "coco.names"])
LABELS = open(labelsPath).read().strip().split("\n")

# YOLO 가중치 및 모델 구성에 대한 경로
weightsPath = os.path.sep.join(["params_", "yolov3.weights"])  # 가중치
configPath = os.path.sep.join(["params_", "yolov3.cfg"])  # 모델 구성
...
while True:
    ...
    for output in layerOutputs:
        ...
        for detection in output:
            ...
            # 사람이 아닌 경우 제외
            #if classID != 0:  # coco.names 파일을 참고하여 다른 object 도 인식 가능(0 인 경우 사람)
            #    continue
    ...
    # counting 결과 출력
    counting_text = " person: {}  Bicycle: {}  Car: {}" \
                    "  Motorbike: {}  Bus: {}  Truck: {}".format(count_person, count_bicycle,
                                                                 count_car, count_motorbike, count_bus, count_truck)
    cv2.putText(frame, counting_text, (10, frame.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (20, 20, 150), 3)


    if accident == 1:
        cv2.circle(frame, (frame.shape[1]-50, 20), 5, (25, 25, 255), -1)
    elif accident == 2:
        cv2.circle(frame, (frame.shape[1] - 50, 20), 5, (25, 255, 0), -1)
